...
describe('chartThemeColors', () => {
  afterEach(() => {
    for (const token of [...TOP_CHART_COLOR_TOKENS, '--muted-foreground']) {
  ...
  describe('getTeamChartColor', () => {
    ...
    it('does not use the legacy monochrome green ramp', () => {
      ...
      expect(OTHERS_CHART_COLOR).toBe('var(--muted-foreground)')
    })
  })
  ...
  describe.each([
    ...
    it('reads a muted Others color from the active palette', () => {
      ...
      expect(readOthersChartColor()).toBe(PALETTE_THEME_VARS[themeKey]['--muted-foreground'])
